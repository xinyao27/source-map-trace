...
class SourceMapTrace {
  ...
  async fromLineAndColumn(line: number, column: number) {
    if (!this.sourceMap)
      throw new Error(`${LOG_PREFIX} fromLineAndColumn failed, please check the sourceMap is correct!`)
  ...
  async fromError(error: ErrorDetail) {
    if (!this.sourceMap)
      throw new Error(`${LOG_PREFIX} fromError failed, please check the sourceMap is correct!`)
